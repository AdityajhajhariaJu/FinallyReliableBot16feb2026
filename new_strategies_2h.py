...
class CMFDivergenceStrategy(BaseStrategy):
    ...
    def evaluate(self, candles: list[Candle]) -> Optional[Signal]:
        ...
        # moderate volume quality filter (not too hard)
        vol_now = candles_5m[-1].volume
        vol_avg = sum(c.volume for c in candles_5m[-20:]) / 20
        vol_ratio = (vol_now / vol_avg) if vol_avg > 0 else 1.0
        if vol_ratio < 0.75:
            return None

        # light context filters (soft boosts, not hard blocks)
        ...
        # confirmation candle (moderate strictness)
        c0 = candles_5m[-1]
        c1 = candles_5m[-2]
        bull_confirm = (c0.close > c0.open and c0.close > c1.high)
        bear_confirm = (c0.close < c0.open and c0.close < c1.low)
        ...
        if (price_recent_low < price_earlier_low
            ...
            conf = 0.64 + min(strength * 2, 0.12) + min((44 - rsi_val) / 170, 0.07)
            if bull_confirm:
                conf += 0.03
            if near_support:
                conf += 0.02
            if trend_down:
                conf += 0.01  # divergence against prior down move
            conf += min(max(vol_ratio - 1.0, 0.0) / 4, 0.02)

            tp = max(0.005, atr_pct * 2.4)
            ...
            return Signal(
                ...
                reason=(f"CMF Bullish Divergence | Price LL but CMF HL"
                        ...
                        f" | Vol={vol_ratio:.1f}x | Cfm={bull_confirm}"),
            )
        ...
        if (price_recent_high > price_earlier_high
            ...
            conf = 0.64 + min(strength * 2, 0.12) + min((rsi_val - 56) / 170, 0.07)
            if bear_confirm:
                conf += 0.03
            if near_resistance:
                conf += 0.02
            ...
            return Signal(
                ...
                reason=(f"CMF Bearish Divergence | Price HH but CMF LH"
                        f" | CMF: {cmf_earlier:.3f}→{cmf_recent:.3f}"
                        f" | RSI={rsi_val:.1f} | Strength={strength:.3f}"
                        f" | Vol={vol_ratio:.1f}x | Cfm={bear_confirm}"),
            )
        return None
...
NEW_2H_STRATEGIES = [
    # IchimokuCloudStrategy(),      # disabled by user
    ...
    # MultiTFEMARibbonStrategy(),   # disabled by user
...
NEW_CATEGORIES = {
    "trend_2h": [
        "donchian_breakout", "supertrend_flip",
        "adx_di_cross",
    ],
    "reversion_2h": [
