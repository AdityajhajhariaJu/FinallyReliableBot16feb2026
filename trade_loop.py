...
from strategies import (
    ...
    set_pair_cooldown, set_strategy_cooldown, get_strategy_cooldown_remaining, load_2h_strategies, calculate_trade_economics,
...
# Strategy-specific entry cooldown overrides (seconds) across all pairs
STRATEGY_ENTRY_COOLDOWN = {
    "cmf_divergence": 21600,  # user requested: after each CMF trade, pause CMF for 6h globally
}

# Strategy-specific post-close cooldown overrides (seconds)
STRATEGY_POST_CLOSE_COOLDOWN = {
    "cmf_divergence": 21600,
}

def _post_close_cooldown_for_strategy(strategy_id: str) -> int:
    if strategy_id in STRATEGY_POST_CLOSE_COOLDOWN:
        return int(STRATEGY_POST_CLOSE_COOLDOWN[strategy_id])
    if is_4h_strategy(strategy_id):
        return int(CONFIG_4H.get("post_close_cooldown_sec", STRAT_CONFIG.get("correlation", {}).get("post_close_cooldown_sec", 0)))
    if is_2h_strategy(strategy_id):
        return int(CONFIG_2H.get("post_close_cooldown_sec", STRAT_CONFIG.get("correlation", {}).get("post_close_cooldown_sec", 0)))
    return int(STRAT_CONFIG.get("correlation", {}).get("post_close_cooldown_sec", 0))
...
# Optional SQLite performance logger
TRADE_DB_PATH = "/opt/multi-strat-engine/reports/trades.db"
LOGGER = None
if TradeLogger:
    try:
        LOGGER = TradeLogger(TRADE_DB_PATH)
    except Exception as _e:
        print(f"TradeLogger init failed: {_e}")
        LOGGER = None
...
def log_filtered(sig, reason):
    ...
    exists = fpath.exists()
    blocked_by = getattr(sig, '_blocked_by', '') or ''
    with fpath.open('a', newline='') as f:
        w = csv.writer(f)
        if not exists:
            w.writerow(["ts","pair","strategy","strategy_id","side","price","reason","blocked_by","outcome","pnl_pct","exit_price","horizon_min"])
        w.writerow([time.time(), sig.pair, sig.strategy_name, getattr(sig, 'strategy_id', ''), sig.side, sig.entry_price, reason, blocked_by, "", "", "", ""])
...
async def place_orders(exchange, signal):
    ...
    # Pre-check market tradability to avoid repeated "Market is closed" order spam
    try:
        m = exchange.market(signal.pair)
        if m and m.get('active') is False:
            print(f"Entry skip {signal.pair} {side}: market inactive")
            return
    except Exception:
        pass
    ...
    try:
        if LOGGER:
            LOGGER.log_signal(signal.pair, signal.strategy_id, signal.side, signal.confidence, signal.entry_price, signal.tp_price, signal.sl_price, signal.leverage, executed=True)
            LOGGER.log_entry(trade_id, normalize_pair(signal.pair), signal.strategy_id, signal.side, float(signal.entry_price or 0), int(signal.leverage or 1), float(amount or 0))
    except Exception as e:
        print(f"TradeLogger entry log error: {e}")
    # Set cooldown only after successful ENTRY (user request)
    try:
        set_pair_cooldown(signal.pair, strategy_id=signal.strategy_id)
    except Exception:
        pass
    # Strategy-level cooldown (global across pairs)
    try:
        scd = int(STRATEGY_ENTRY_COOLDOWN.get(signal.strategy_id, 0) or 0)
        if scd > 0:
            set_strategy_cooldown(signal.strategy_id, scd)
    except Exception:
        pass
    ...
    try:
        ...
        try:
            if trade_id:
                ...
                if LOGGER:
                    LOGGER.log_exit(trade_id, close_price, pnl, 0.0, "MAX_AGE")
        except Exception:
            pass
        try:
            post_close = _post_close_cooldown_for_strategy(sid or "")
            cooldown = STRAT_CONFIG.get("correlation", {}).get("cooldown_sec", 0)
            extra = max(0, int(post_close) - int(cooldown))
            set_pair_cooldown(pair, time.time() + extra if extra else time.time(), strategy_id=(sid or ""))
        except Exception:
            pass
...
async def finalize_disappeared_position(exchange, pair, side):
    ...
    try:
        ...
        try:
            update_trade_exit(trade_id, time.time(), close_price, pnl)
            if LOGGER:
                LOGGER.log_exit(trade_id, close_price, pnl, 0.0, "CLOSE_DETECTED")
        except Exception:
            pass
    except Exception:
        pass
...
async def loop():
    ...
    try:
        while True:
            start = time.time()
            pairs_1m = list(PAIRS)
            pairs_htf = await get_top_pairs_for_htf(exchange, top_n=40)
            scan_pairs = sorted(set(pairs_1m) | set(pairs_htf))
            STRAT_CONFIG["pairs_2h"] = list(pairs_htf)
            STRAT_CONFIG["pairs_4h"] = list(pairs_htf)

            tasks_1m = [fetch_candles(exchange, p, TIMEFRAME, limit=FETCH_LIMIT) for p in scan_pairs]
            tasks_5m = [fetch_candles(exchange, p, TIMEFRAME_5M, limit=FETCH_LIMIT) for p in scan_pairs]
            tasks_15m = [fetch_candles(exchange, p, TIMEFRAME_15M, limit=FETCH_LIMIT) for p in scan_pairs]
            tasks_1h = [fetch_candles(exchange, p, TIMEFRAME_1H, limit=500) for p in pairs_htf]
            ...
            try:
                ...
                base_cooldown = STRAT_CONFIG.get("correlation", {}).get("cooldown_sec", 0)
                for pair in list(position_meta.keys()):
                    if pair not in current_pairs:
                        # Position disappeared (likely TP/SL/manual close): finalize entry log exit row.
                        sid_for_cd = (position_meta.get(pair) or {}).get('strategy_id') or ''
                        try:
                            side_guess = 'LONG'
                            row = _resolve_open_trade_from_log(pair)
                            if row and row.get('side'):
                                side_guess = (row.get('side') or 'LONG').upper()
                            if not sid_for_cd and row:
                                sid_for_cd = row.get('strategy_id') or ''
                            await finalize_disappeared_position(exchange, pair, side_guess)
                        except Exception:
                            pass
                        post_close = _post_close_cooldown_for_strategy(sid_for_cd)
                        extra = max(0, int(post_close) - int(base_cooldown))
                        ts = time.time() + extra if extra else time.time()
                        set_pair_cooldown(pair, ts, strategy_id=(sid_for_cd or ""))
                        position_meta.pop(pair, None); save_position_meta()
                        _pending_entries.discard(pair)  # BUG-010 FIX
                        print(f"Post-close cooldown set for {pair} ({post_close}s) strat={sid_for_cd or 'unknown'}")
            ...
            try: news_bias = get_news_bias(scan_pairs, pos_bias=0.02, neg_bias=0.04, half_life_hours=2.0)
                ...
                for s in after_category[slots_available:]:
                    try:
                        if float(getattr(s,'confidence',0) or 0) >= 0.66:
                            log_high_conf_drop(s, getattr(s, '_filter_reason', '') or "slot_limit")
                    except Exception:
                        pass

            # Optional SQLite signal logging (executed flag set in place_orders)
            if LOGGER:
                try:
                    for s in res.signals:
                        LOGGER.log_signal(s.pair, s.strategy_id, s.side, s.confidence, s.entry_price, s.tp_price, s.sl_price, s.leverage, executed=False)
                    for fs in res.filtered:
                        LOGGER.log_signal(fs.pair, fs.strategy_id, fs.side, fs.confidence, fs.entry_price, fs.tp_price, fs.sl_price, fs.leverage, executed=False, filter_reason=(fs._filter_reason or ""))
                except Exception as e:
                    print(f"TradeLogger signal log error: {e}")
            ...
            if res.drawdown and res.drawdown.paused:
                ...
                for sig in res.signals:
                    # Re-check strategy cooldown at execution time so only first signal can pass.
                    try:
                        rem = float(get_strategy_cooldown_remaining(sig.strategy_id) or 0)
                    except Exception:
                        rem = 0
                    if rem > 0:
                        print(f"SKIP {sig.pair}: strategy cooldown active for {sig.strategy_id} ({rem:.0f}s left)")
                        continue
